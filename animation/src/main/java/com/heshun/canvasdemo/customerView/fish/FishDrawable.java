...
public class FishDrawable extends Drawable {
    ...
    private void init() {
        ...
        ValueAnimator valueAnimator = ValueAnimator.ofInt(0, 540);
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                currentPercent= (int) (animation.getAnimatedFraction()*540);
                Log.e(TAG, "onAnimationUpdate: "+currentPercent );
                invalidateSelf();
            }
        });
        valueAnimator.start();
    ...
    private void makeBody(Canvas canvas, float headRadius) {
        float angle = fatherAngle + (float) Math.sin(Math.toRadians(currentPercent * 1.2 * waveFrequence)) * 2;//中心轴线和X轴顺时针方向夹角
    ...
    private void makeTail(Canvas canvas, PointF mainPoint, float length, float maxWidth, float angle) {
        //TODO 鱼尾宽度控制
//        currentPercent += 15;
        float newWidth = (float) Math.abs(Math.sin(Math.toRadians(currentPercent * 1.7 * waveFrequence)) * maxWidth + 15);
//        if (currentPercent > 54000) {
//            currentPercent = 0;
//        }
    ...
    public int getIntrinsicHeight() {
        return (int) ((TOTAL_LENGTH + BODY_LENGHT / 2) * 2 + 0.5);
    }
