...
public class LocalFileManager {
	...
	public final void deleteAllFiles(File dir) {
		...
		if (exceptionOccurred) {
			Log.e(TAG, "deleteAllFiles: 部分文件删除失败" );
		}
	}
	...
	public final void cleanCache(Context context) {
		deleteAllFiles(FileUtil.getCacheRootFile(context));
	}
	...
	public final String getCacheSize(Context context) {
		return getPathSize(FileUtil.getCacheRootFile(context).getAbsolutePath());
	}
	...
	public boolean checkSdCard() {
		...
		if (sdCardState.equals(Environment.MEDIA_REMOVED)) {// SdCard已移动
			Toast.makeText(mAppContext, "SdCard已移动", Toast.LENGTH_SHORT).show();
			return false;
		} else if (sdCardState.equals(Environment.MEDIA_CHECKING)) {
			Toast.makeText(mAppContext,"检查中", Toast.LENGTH_SHORT).show();
			return false;
		}
		if (!sdCardState.equals(Environment.MEDIA_MOUNTED)) {// SdCard未挂载
			Toast.makeText(mAppContext, "SdCard未挂载", Toast.LENGTH_SHORT).show();
			return false;
		}
		return true;
	}
