...
/**
 * 两个bitmap内容完全被填充填充但是相互不对齐
 * author：Jics
 * 2017/7/24 11:06
 */
public class XferView2_r extends View {
...
//		c.drawRect(w/3, h/3, w*19/20, h*19/20, p);
	...
	public XferView2_r(Context context) {
		super(context);

		init();
	}

	public XferView2_r(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < sModes.length; i++) {
			...
			//后画的是源src
			canvas.drawBitmap(mRectB, W*0.2f, W*0.2f, paint);//方
